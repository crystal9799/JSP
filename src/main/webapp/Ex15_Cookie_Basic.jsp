...
1. Cookie		(메모리 쿠키 , 파일 쿠키 (local 디스크에 txt 암호화 시켜서 저장 : 소멸타임))
   단점) 크기가 제한적 (4096byte) , 보안 (사용자가 삭제 , 해커 파일 노출 위험)
2. Local Storage(key , value) 형태 > JSON 형태로 READ, WRITE
   장점) 크기 , 객체 사용(데이터 : 의미있는 데이터 저장)
   
Server (웹 서버 (WAS) : 메모리 , 파일 , DB서버)
1. server memory : session 객체 (사이트에 접속하는 [사용자]마다 [고유]하게 부여되는 메모리(객체) > 서버 리뷰팅 , 개발 코드 변경 > 소멸 > 생성
2. server momory : application 객체 (사이트에 접속하는 [모든 사용자]에게 부여되는 메모리(객체) > 서버 리뷰팅 , 개발 코드 변경 > 소멸 > 생성
